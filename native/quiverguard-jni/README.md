...
[adblock-rust](https://github.com/brave/adblock-rust), pinned to `=0.13.2`.
...
The `adblock` crate is pinned to exactly `0.13.2` rather than using a caret
...
### Upstream changelog since the last pin (0.13.0 -> 0.13.2)

From upstream's `CHANGELOG.md`:

- **0.13.1** - Fixed: `adblock::engine::DeserializationError` is now exported.
  Not referenced by this crate (`nativeLoadEngine` only checks
  `deserialize(...).is_err()`), no change needed.
- **0.13.2** - Added: `Default` impl for `CosmeticFilterCache`. Not used here.
- **0.13.2** - Changed: `Engine::set_regex_discard_policy` now takes `&self`
  instead of `&mut self`. This crate doesn't call that method, no change needed.
- **0.13.2** - Changed: reduced allocation count in
  `Engine::hidden_class_id_selectors`. Same signature, used as-is in
  `nativeHiddenClassIdSelectors` - no source change, just faster.
